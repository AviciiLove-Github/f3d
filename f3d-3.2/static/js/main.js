...
async function processSkeletonBeforeChat() {
    ...
    const skeleton = getChangedBoneData();
    if (skeleton.length === 0) return; // 没有变化的骨骼，直接返回
    ...
    try {
        ...
        const response = await fetch('/api/send-to-skeleton', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ skeleton: formattedText }),
        });
        if (response.ok) {
            const result = await response.json();
            if (result.success) {
                const resultText = result.result !== undefined ? String(result.result) : '';
                if (resultText) {
                    addMessage(`骨骼处理完成，已发送 ${skeleton.length} 个发生变化的骨骼。\n\n处理结果：${resultText}`, false);
                } else {
                    addMessage(`骨骼处理完成，已发送 ${skeleton.length} 个发生变化的骨骼。`, false);
                }
                window.lastSkeletonProcessed = true;
            } else {
                addMessage(`骨骼处理失败：${result.error}`, false);
            }
        } else {
            throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        }
        ...
        if (response.ok) {
            ...
            if (result.success) {
                ...
                if (resultText) {
                    ...
                    addMessage(`骨骼处理成功，已发送 ${skeleton.length} 个发生变化的骨骼。`, false);
                }
                // 标记骨骼已处理
                window.lastSkeletonProcessed = true;
            } else {
                addMessage('骨骼处理失败：' + (result.error || '未知错误'), false);
            }
        } else {
            addMessage('骨骼处理请求失败：' + response.statusText, false);
        }
    } catch (err) {
