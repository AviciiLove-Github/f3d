...
def sendToModel(inputText, history):
    '''
    history[0]['content'] = 'prompt:你是一个AI医生。' + history[0]['content']
    ...
    try:
        ...
        with open('Temp-S', 'r', encoding='utf-8') as f:
            temp_s = f.read()
        history[-1]['content'] += temp_s
    except:None

    client = OpenAI(
        base_url='https://qianfan.baidubce.com/v2',
        api_key='bce-v3/ALTAK-67rf8p0KFOR2CCUWCrYyk/b6e312200bb2cd07dfc762692d7b5d4e7feb678f'
    )
...
@app.route('/api/send-to-skeleton', methods=['POST'])
def send_to_skeleton():
    """接收前端传来的骨骼数据，调用 sendToSkeleton（仅传入改变了的骨骼）"""
    try:
        data = request.get_json()
        skeleton = data.get('skeleton', '')
        result = sendToSkeleton(skeleton)
        return jsonify({
            'success': True,
            'message': '骨骼数据处理成功',
            'result': result
